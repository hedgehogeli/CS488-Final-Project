...
bool Image::loadPng(const std::string & filename)
{
    ...
    // Decode as RGBA (lodepng default), then we'll extract RGB
    unsigned error = lodepng::decode(rawPixels, w, h, filename, LCT_RGB);
    ...
    // Convert from [0,255] unsigned char to [0,1] double
    for (uint y = 0; y < m_height; ++y) {
        for (uint x = 0; x < m_width; ++x) {
            for (uint i = 0; i < m_colorComponents; ++i) {
                unsigned char c = rawPixels[m_colorComponents * (m_width * y + x) + i];
                m_data[m_colorComponents * (m_width * y + x) + i] = c / 255.0;
            }
        }
    }
