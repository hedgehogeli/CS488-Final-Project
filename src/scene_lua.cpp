...
static RenderPreset presetFromLuaQuality(const char* q)
{
  if (q != nullptr && std::strcmp(q, "low") == 0) {
    return RenderPreset::lowQuality();
  }
  return RenderPreset::highQuality();
}
...
// Create a Cylinder node (canonical capped cylinder in local space; transform with the node)
extern "C"
...
int gr_render_cmd(lua_State* L)
{
  ...
  if (lua_gettop(L) >= 11 && lua_type(L, 11) == LUA_TSTRING) {
    quality = lua_tostring(L, 11);
  }
  RenderPreset preset = presetFromLuaQuality(quality);

	Image im( width, height);
	A5_Render(root->node, im, eye, view, up, fov, ambient, lights, preset);
    im.savePng( filename );
...
// Create a Material
// gr.material(kd, ks, shininess [, transparency [, refractiveIndex]])
extern "C"
int gr_material_cmd(lua_State* L)
{
  ...
  // Optional args 4–5: use only if present and numeric (avoid stack garbage/userdata)
  double transparency = (lua_gettop(L) >= 4 && lua_type(L, 4) == LUA_TNUMBER) ? lua_tonumber(L, 4) : 0.0;
  double refractiveIndex = (lua_gettop(L) >= 5 && lua_type(L, 5) == LUA_TNUMBER) ? lua_tonumber(L, 5) : 0.0;
...
// Create a participating medium material.
// gr.medium_material(scatteringColour, sigmaA, sigmaS [, densityScale [, stepSize]])
extern "C"
int gr_medium_material_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double scatteringColour[3], sigmaA[3], sigmaS[3];
  get_tuple(L, 1, scatteringColour, 3);
  get_tuple(L, 2, sigmaA, 3);
  get_tuple(L, 3, sigmaS, 3);

  const int n = lua_gettop(L);
  const float densityScale =
      (n >= 4 && lua_type(L, 4) == LUA_TNUMBER) ? static_cast<float>(lua_tonumber(L, 4)) : 1.0f;
  const float stepSize =
      (n >= 5 && lua_type(L, 5) == LUA_TNUMBER) ? static_cast<float>(lua_tonumber(L, 5)) : 0.1f;

  data->material = new MediumMaterial(
      glm::vec3(scatteringColour[0], scatteringColour[1], scatteringColour[2]),
      glm::vec3(sigmaA[0], sigmaA[1], sigmaA[2]),
      glm::vec3(sigmaS[0], sigmaS[1], sigmaS[2]),
      densityScale,
      stepSize);
...
/*
  Create a material with diffuse (*_diff_*.png) and optional normal GL (*_nor_gl_*.png) maps.

  gr.textured_material(kd, ks, shininess, diffuse_path, normal_path
                       [, uv_scale [, transparency [, refractiveIndex]]])

  - kd, ks: same as gr.material (3-tuples)
  - shininess: Phong exponent
  - diffuse_path: PNG path, or "" to skip diffuse map
  - normal_path: PNG path, or "" to skip normal map (bump mapping uses this later)
  - uv_scale: optional {sx, sy} for tiling / aspect (default {1, 1})
  - transparency, refractiveIndex: same optional trailing args as gr.material

  Example (paths relative to cwd when you run the executable):

    wood = gr.textured_material(
      {0.9, 0.85, 0.7}, {0.2, 0.2, 0.2}, 32,
      "Assets/wood_diff_4k.png",
      "Assets/wood_nor_gl_4k.png",
      {2.0, 1.0}
    )

    diffuse_only = gr.textured_material(
      {1, 1, 1}, {0.1, 0.1, 0.1}, 10,
      "Assets/brick_diff.png",
      ""
    )
*/
extern "C"
int gr_textured_material_cmd(lua_State* L)
{
  ...
  double shininess = luaL_checknumber(L, 3);
  const char* diffuse_path = luaL_checkstring(L, 4);
  const char* normal_path = luaL_checkstring(L, 5);
  ...
  const int n = lua_gettop(L);
  if (n >= 6 && lua_type(L, 6) == LUA_TTABLE) {
    double s[2];
    get_tuple(L, 6, s, 2);
    uvScale = glm::vec2(static_cast<float>(s[0]), static_cast<float>(s[1]));
  }
  if (n >= 7 && lua_type(L, 7) == LUA_TNUMBER) {
    transparency = lua_tonumber(L, 7);
  }
  if (n >= 8 && lua_type(L, 8) == LUA_TNUMBER) {
    refractiveIndex = lua_tonumber(L, 8);
  }

  data->material = new TexturedMaterial(
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"medium_material", gr_medium_material_cmd},
